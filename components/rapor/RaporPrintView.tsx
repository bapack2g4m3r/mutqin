...
export default function RaporPrintView({ siswaId, backUrl }: RaporPrintViewProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/siswa/${siswaId}/rapor`)
      .then(async res => {
        const d = await res.json()
        if (!res.ok || d.error) {
          throw new Error(d.error || 'Gagal memuat data rapor.')
        }
        setData(d)
      })
      .catch(err => {
        setErrorMsg(err.message || 'Terjadi kesalahan saat memuat rapor.')
      })
  ...
  return (
    <>
      ...
      <button className="btn-back no-print" onClick={handleBack}>
        ...
        <table className="info-siswa">
          <tbody>
            ...
            <tr>
              ...
              <td className="colon">:</td>
              <td className="val-right" style={{ textTransform: 'capitalize' }}>
                {data.semester ? data.semester.toLowerCase() : 'Ganjil'}
              </td>
            </tr>
            <tr>
